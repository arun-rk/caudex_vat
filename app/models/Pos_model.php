...
class Pos_model extends CI_Model
{
    ...
    public function getProductNames($term, $limit = 10)
    {
        ...
        $this->db->select('products.*, uom.symbol as symbol')
            ...
            // 
        $this->db->select("{$this->db->dbprefix('products')}.*, COALESCE(psq.quantity, 0) as quantity, COALESCE(psq.price, 0) as store_price,uom.symbol as symbol")
            ->join("( SELECT * from {$this->db->dbprefix('product_store_qty')} WHERE store_id = {$store_id}) psq", 'products.id=psq.product_id', 'inner');
        // 		->join('purchase_items', 'purchase_items.product_id=products.id', 'inner')
        //     ->join('uom', 'uom.id=products.uom_id', 'left');
        //    ->join('purchases', 'purchases.id=purchase_items.purchase_id', 'inner');
        if ($this->db->dbdriver == 'sqlite3') {
            $this->db->where("(name LIKE '%{$term}%' OR code LIKE '%{$term}%' OR local_barcode LIKE '%{$term}%' OR  (name || ' (' || code || ')') LIKE '%{$term}%')");
        } else {
            $this->db->where("(name LIKE '%{$term}%' OR code LIKE '%{$term}%' OR local_barcode LIKE '%{$term}%' OR  concat(name, ' (', code, ')') LIKE '%{$term}%')");
        }
        $this->db->group_by('products.id')->limit($limit);
				$q = $this->db->get('products');
				
		$qr = $this->db->queries;
		$qr1 = $this->db->last_query();
		$qr = $this->db->error();
        if ($q->num_rows() > 0) {
    ...
    public function getProductByCode($code)
    {
        ...
        $this->db->select("{$this->db->dbprefix('products')}.*, COALESCE(PSQ.quantity, 0) as quantity, COALESCE(PSQ.price, {$this->db->dbprefix('products')}.price) as store_price, , uom.symbol as symbol", false)
...
$x = $this->db->error();
    ...
    public function estimateSale($data, $items, $did = null)
    {

        if ($did) {

            if ($this->db->update('estimate_sales', $data, array('id' => $did)) && $this->db->delete('suspended_items', array('estimate_id' => $did))) {
                foreach ($items as $item) {
                    unset($item['cost']);
                    $item['estimate_id'] = $did;
                    $this->db->insert('estimate_items', $item);
                }
                return true;
            }
            ...
            if ($this->db->insert('estimate_sales', $data)) {
                echo JSON_ENCODE('A');
                $suspend_id = $this->db->insert_id();
                foreach ($items as $item) {
                    unset($item['cost']);
                    $item['estimate_id'] = $suspend_id;
                    $this->db->insert('estimate_items', $item);
                }
                return $suspend_id;
            }
    ...
    public function getEstimateID($sale_id)
    {
        $q = $this->db->get_where('estimate_sales', array('id' => $sale_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
				$q = $this->db->get_where('sales_returns', array('id' => $sale_id), 1);
				// $this->db->select('tec_sales_returns.*,tec_uom.base_name as unit');
				// $this->db->from('tec_sales_returns');
				// $this->db->where('tec_sales_returns.id', $sale_id);
				// $this->db->join('tec_sale_items_returns', ' tec_sale_items_returns.sale_id = tec_sales_returns.id');
				// $this->db->join('tec_products', 'tec_products.id = tec_sale_items_returns.product_id');
				// $this->db->join('tec_uom', 'tec_uom.id = tec_products.uom_id');
				// $this->db->join('tec_products', '  = tec_products.id');
				// $q = $this->db->get();
				$er = $this->db->error();
        if ($q->num_rows() > 0) {
            return $q->row();
    ...
    public function getAllSaleItems($sale_id)
    {
        ...
        $this->db->select("sale_items.*," . $this->db->dbprefix('products') . ".hsncode as hsn,
            ...
            {$this->db->dbprefix('uom')}.symbol as unit , ", false)
            ->join('products', 'products.id=sale_items.product_id', 'inner')
            ->join('tec_uom', 'tec_uom.id=products.uom_id', 'left')
    ...
    public function getAllEstimateItems($sale_id)
    {
        $j = "(SELECT id, code, name, tax_method from {$this->db->dbprefix('products')}) P";
        $this->db->select("estimate_items.*," . $this->db->dbprefix('products') . ".hsncode as hsn,
            (CASE WHEN {$this->db->dbprefix('estimate_items')}.product_code IS NULL THEN {$this->db->dbprefix('products')}.code ELSE {$this->db->dbprefix('estimate_items')}.product_code END) as product_code,
            (CASE WHEN {$this->db->dbprefix('estimate_items')}.product_name IS NULL THEN {$this->db->dbprefix('products')}.name ELSE {$this->db->dbprefix('estimate_items')}.product_name END) as product_name,
            {$this->db->dbprefix('products')}.tax_method as tax_method", false)
            ->join('products', 'products.id=estimate_items.product_id', 'inner')
            ->order_by('estimate_items.id');
        $q = $this->db->get_where('estimate_items', array('estimate_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllSaleReturnItems($sale_id)
    {
        $j = "(SELECT id, code, name, tax_method from {$this->db->dbprefix('products')}) P";
        $this->db->select("sale_items_returns.*,tec_products.hsncode as hsn,
            (CASE WHEN {$this->db->dbprefix('sale_items_returns')}.product_code IS NULL THEN {$this->db->dbprefix('products')}.code ELSE {$this->db->dbprefix('sale_items_returns')}.product_code END) as product_code,
            (CASE WHEN {$this->db->dbprefix('sale_items_returns')}.product_name IS NULL THEN {$this->db->dbprefix('products')}.name ELSE {$this->db->dbprefix('sale_items_returns')}.product_name END) as product_name,
            {$this->db->dbprefix('products')}.tax_method as tax_method, tec_uom.base_name as unit", false)
            ->join('products', 'products.id=sale_items_returns.product_id', 'left outer')
            ->join('uom', 'uom.id=products.uom_id', 'left outer')
            ->order_by('sale_items_returns.id');
        $q = $this->db->get_where('sale_items_returns', array('sale_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllSalePaymentsReturn($sale_id)
    {
        $q = $this->db->get_where('payments_returns', array('sale_id' => $sale_id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getQutationSaleByID($id)
    {
        $q = $this->db->get_where('estimate_sales', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getQutationSaleItems($id)
    {
        $q = $this->db->get_where('estimate_items', array('estimate_id' => $id));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getBillId($qs='')
    {
			if($qs){
        $q = $this->db->query("SELECT id,customer_name FROM tec_sales WHERE id LIKE ? ",array($qs.'%'));
        if (($q) && ($q->num_rows())) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
		...
		public function update_customer_balance($duedetails)
		{
		...
		public function has_creditlimit_reached($id,$current_amt) {
			if($q=$this->db->query('SELECT SUM(grand_total)-SUM(paid) AS outstanding_amt FROM  tec_sales
			WHERE customer_id = ?',array($id))) 
			{
				$q1=$this->db->query('SELECT credit_limit FROM  tec_customers
						WHERE id = ?',array($id));
						if($q1->row()->credit_limit){
							if($q1->row()->credit_limit <= ($q->row()->outstanding_amt+ $current_amt)){
								return true;
							}else{
								return false;
							}
	...
	public function GetCounterDetails($CounterId = NULL) {

		$q = $this->db->get_where('counter', array('id' => $CounterId), 1);
		if ($q->num_rows() > 0) {
				return $q->row();
		}
		return FALSE;
