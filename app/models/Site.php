...
class Site extends CI_Model
{
    ...
    public function getGstByID($id) {
        $q = $this->db->get_where('gst_groups', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
...
public function getAllCategories_POS() {
	$this->db->order_by('code');

	$q = $this->db->get_where('categories', array('IsStore'=>0));
	if ($q->num_rows() > 0) {
			foreach (($q->result()) as $row) {
					$data[] = $row;
			}
