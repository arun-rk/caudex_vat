...
class Reports_model extends CI_Model
{
    ...
    public function topProducts() {
        ...
        ->limit(10)
        ->where('YEAR(tec_sales.date)', $d[0])
        ->where('MONTH(tec_sales.date)', $d[1]);
        // ->like('sales.date', $d, 'both');
    ...
    public function getDailySales($year, $month) {
        ...
        $this->db->like('year(date)', "{$year}", 'after');
        $this->db->like('month(date)', "{$month}", 'after');
    ...
    public function getTotalCustomerSales($customer_id, $user = NULL, $start_date = NULL, $end_date = NULL) {
	 ...
	 public function getGSTReports($start_date = NULL, $end_date = NULL) {
        $query=$this->db->query("SELECT date(`date`) as 'Invoice_Date',count(s.Id) as 'Total_Inv',sum(sl.item_discount) as 'Discount', sl.cgst_tax as 'CGST',sum(sl.cgst_tax_val) as 'CGST_Amt', sl.sgst_tax as 'SGST' ,sum(sl.sgst_tax_val) as 'SGST_Amt' from tec_sales as s INNER JOIN tec_sale_items as sl on s.id=sl.sale_id group by date(`date`)");
        /* if ($start_date && $end_date) {
            $this->db->where('date >=', $start_date);
            $this->db->where('date <=', $end_date);
        }
