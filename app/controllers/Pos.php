...
class Pos extends MY_Controller
{
    ...
    public function index($sid = null, $eid = null)
    {
        ...
        if ($this->input->get('qut')) {$qut = $this->input->get('qut');}
        ...
        $estimate = $this->input->post('estimate') ? true : false;
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                    ...
                    $pr_cgst_item_tax = 0;
                    $pr_sgst_item_tax = 0;
                    $gst_details = $this->site->getGstByID($product_details->gst_id);
                    ...
                    $cgst_tax = $gst_details->cgst . "%";
                    $sgst_tax = $gst_details->sgst . "%";
                    $cgst_value = (($unit_price) * $gst_details->cgst) / 100;
                    $sgst_value = (($unit_price) * $gst_details->sgst) / 100;
                    // $cgst_value = $this->tec->formatDecimal((($unit_price) * $gst_details->cgst) / 100);
                    // $sgst_value = $this->tec->formatDecimal((($unit_price) * $gst_details->sgst) / 100);
                    // $pr_cgst_item_tax = $this->tec->formatDecimal($cgst_value * $item_quantity);
                    // $pr_sgst_item_tax = $this->tec->formatDecimal($sgst_value * $item_quantity);
                    $pr_cgst_item_tax = $cgst_value * $item_quantity;
                    $pr_sgst_item_tax = $sgst_value * $item_quantity;
                    //               } else {
                    ...
                    $cgst_total_tax += $pr_cgst_item_tax;
                    $sgst_total_tax += $pr_sgst_item_tax;
											if($product_details->tax_method == "0"){
												$subtotal = (($item_net_price * $item_quantity));
											}
											else{
												$subtotal = (($item_net_price * $item_quantity) + $pr_cgst_item_tax + $pr_sgst_item_tax);
											}
                    ...
                    $products[] = array(
                        ...
                        'cgst_tax' => $cgst_tax,
                        'sgst_tax' => $sgst_tax,
                        'cgst_tax_val' => $pr_cgst_item_tax,
                        'sgst_tax_val' => $pr_sgst_item_tax,
                        'subtotal' => $subtotal,
            ...
            if ($this->input->post('order_discount')) {
                ...
                if ($opos !== false) {
                    ...
                    $order_discount = $this->tec->formatDecimal((($total + $cgst_total_tax + $sgst_total_tax) * (Float) ($ods[0])) / 100);
                } else {
            ...
            if ($this->input->post('order_tax')) {
                ...
                if ($opos !== false) {
                    ...
                    $order_tax = $this->tec->formatDecimal((($total + $cgst_total_tax + $sgst_total_tax - $order_discount) * (Float) ($ots[0])) / 100);
                } else {
                ...
                $order_tax = 0;
						...
						$total_tax = $this->tec->formatDecimal($cgst_total_tax + $sgst_total_tax);
						if($product_details->tax_method == "0"){
							$grand_total = $this->tec->formatDecimal($this->tec->formatDecimal($total) - $order_discount);
						}
						else{
							$grand_total = (float)$this->tec->formatDecimal($total) + (float)$total_tax - $order_discount;
						
							// $grand_total = (float)$this->tec->formatDecimal($total) + $cgst_total_tax + $sgst_total_tax - $order_discount;
							// $grand_total = (float)$this->tec->formatDecimal($this->tec->formatDecimal($total) + $cgst_total_tax + $sgst_total_tax - $order_discount);
							//arcode

							//
						}
						if(!$this->input->post('gst_no_ar')){
							$kerala_flood_cess = $grand_total * 0.01;
						//	$grand_total +=round($kerala_flood_cess);
						}
						// $grand_total = $this->tec->roundNumber($grand_total, $this->Settings->rounding);
						$x =$this->input->post('amount');

						$grand_total = $grand_total *1000;
						$round_total = round($grand_total/5)*5;
						$rounding = $round_total - $grand_total;
						$rounding = $rounding/1000;
						$grand_total = $grand_total/1000;
						$round_total = $round_total/1000;

            $paid = $this->input->post('amount') ? $this->input->post('amount') : 0;
            if (!$eid) {
                $status = 'due';
                if ($round_total > $paid && $paid > 0) {
                    $status = 'partial';
                } elseif ($round_total <= $paid) {
                    $status = 'paid';
                }
						}
						if($status == 'due'){
							$due_details['total_balance'] = $round_total;
							$due_details['id'] = $customer_details->id;
						}else if($status == 'paid'){
							$due_details['total_balance'] = ($round_total*1000 - $paid*1000)/1000;
							$due_details['id'] = $customer_details->id;
            ...
            // $round_total = $this->tec->roundNumber($grand_total, $this->Settings->rounding);
						...
						// $round_total1 = (string)$round_total;
						// $r = $round_total1 - $paid;
						$r = $round_total*1000 -  $paid*1000;
						$r = $r/1000;
						// die($paid);
            if ($this->pos_model->has_creditlimit_reached($customer_details->id,$round_total)) {
                $this->session->set_flashdata('error', lang('Customer credit has reached !'));
                redirect($_SERVER["HTTP_REFERER"]);
						}
						
            if ($customer_details->id == 1 && ($r >0)) {
            ...
            if (!$eid && !$suspend && $paid && !$estimate) {
                $amount = $this->tec->formatDecimal($paid > $round_total ? ($paid - $this->input->post('balance_amount')) : $paid);
                $payment = array(
                    'date' => $date,
                    'amount' => $amount,
                    'customer_id' => $customer_id,
        ...
        if ($this->form_validation->run() == true && !empty($products)) {
            if ($suspend) {
                ...
                if ($this->pos_model->estimateSale($data, $products, $did)) {
                    $this->session->set_userdata('rmspos', 1);
                    $this->session->set_flashdata('message', lang("Quotation saved successfully !"));
                    redirect("pos");
                } else {
                ...
                $data['cash_amount']=$this->input->post('cash_amount_val');
                $data['card_amount']=$this->input->post('card_amount_val');
                $data['cheque_amount']=$this->input->post('cheque_amount_val');

                if ($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
									// due_details
										$res = $this->pos_model->update_customer_balance($due_details);
                    $this->session->set_userdata('rmspos', 1);
                    $msg = lang("sale_added");
            ...
            if (isset($sid) && !empty($sid)) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'gst' => $gst);
                    $c++;
                }
            ...
            if (isset($qut) && !empty($qut)) {
                $suspended_sale = $this->pos_model->getQutationSaleByID($qut);
                $inv_items = $this->pos_model->getQutationSaleItems($qut);
            ...
            $this->data['reference_note'] = isset($qut) && !empty($qut) ? $suspended_sale->hold_ref : (isset($eid) && !empty($eid) ? $sale->hold_ref : null);
            $this->data['sid'] = isset($sid) && !empty($sid) ? $sid : 0;
            $this->data['qut'] = isset($qut) && !empty($qut) ? $qut : 0;
    ...
    public function get_product($code = null)
    {
        ...
        if ($product = $this->pos_model->getProductByCode($code)) {
            ...
            $gst = $this->site->getGstByID($product->gst_id);

            echo json_encode(array('id' => str_replace(".", "", microtime(true)), 'item_id' => $product->id, 'label' => $product->name . " (" . $product->code . ")", 'row' => $product, 'combo_items' => $combo_items, 'gst' => $gst));
        } else {
            echo null;
        }
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'gst' => $gst);
            }
            echo json_encode($pr);
        } else {
    ...
    public function quta($sale_id = null, $noprint = null)
    {
        if ($this->input->get('id')) {$sale_id = $this->input->get('id');}
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['message'] = $this->session->flashdata('message');
        $inv = $this->pos_model->getEstimateID($sale_id);
        if (!$this->session->userdata('store_id')) {
            $this->session->set_flashdata('warning', lang("please_select_store"));
            redirect('stores');
        } elseif ($this->session->userdata('store_id') != $inv->store_id) {
        ...
        $this->tec->view_rights($inv->created_by);
        $this->load->helper('text');
        $this->data['rows'] = $this->pos_model->getAllEstimateItems($sale_id);
        $this->data['customer'] = $this->pos_model->getCustomerByID($inv->customer_id);
        $this->data['store'] = $this->site->getStoreByID($inv->store_id);
        $this->data['inv'] = $inv;
        $this->data['sid'] = $sale_id;
        ...
        $this->data['store'] = $this->site->getStoreByID($inv->store_id);
        $this->data['page_title'] = lang("invoice");
        $this->load->view($this->theme . 'pos/' . ($this->Settings->print_img ? 'eview' : 'quta'), $this->data);

    }

    public function returnview($sale_id = null, $noprint = null)
    {
        if ($this->input->get('id')) {$sale_id = $this->input->get('id');}
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['message'] = $this->session->flashdata('message');
        $inv = $this->pos_model->getReturnSaleByID($sale_id);
        if (!$this->session->userdata('store_id')) {
            $this->session->set_flashdata('warning', lang("please_select_store"));
            redirect('stores');
        } elseif ($this->session->userdata('store_id') != $inv->store_id) {
        ...
        $this->tec->view_rights($inv->created_by);
        $this->load->helper('text');
        $this->data['rows'] = $this->pos_model->getAllSaleReturnItems($sale_id);
        $this->data['customer'] = $this->pos_model->getCustomerByID($inv->customer_id);
        $this->data['store'] = $this->site->getStoreByID($inv->store_id);
        $this->data['inv'] = $inv;
        $this->data['sid'] = $sale_id;
        ...
        $this->data['noprint'] = $noprint;
        $this->data['modal'] = $noprint ? true : false;
        $this->data['payments'] = $this->pos_model->getAllSalePaymentsReturn($sale_id);
        $this->data['created_by'] = $this->site->getUser($inv->created_by);
        $this->data['printer'] = $this->site->getPrinterByID($this->Settings->printer);
        $this->data['store'] = $this->site->getStoreByID($inv->store_id);
        $this->data['page_title'] = lang("invoice");
    ...
    public function print_receipt($id=1, $open_drawer = false)
    {
    ...
    public function p($bo = 'order')
    {
		...
		{
			$q = $this->input->get('id');
			$data = $this->pos_model->getBillId($q);
			$list = array();
			if($data){
				foreach ($data as $value) {
					$item = new stdClass();
					$item->id = $value->id;
					$item->text = $value->id.'('.$value->customer_name.')';
...
// cake world integration start
function pos_screen($sid = NULL, $eid = NULL)
{
 ...
 $cancel='';
		 ...
		 if( $this->input->get('edit') ) { $eid = $this->input->get('edit'); }
		 if( $this->input->post('eid') ) { $eid = $this->input->post('eid'); }
 if( $this->input->post('Last_Pay') ) { $Last_Pay = $this->input->post('Last_Pay'); }
		 if( $this->input->post('did') ) { $did = $this->input->post('did'); } else { $did = NULL; }
	 if( $this->input->post('cancel') ) { $cancel = $this->input->post('cancel'); }


		 /* if($eid && !$this->Admin) {
				 $this->session->set_flashdata('error', lang('access_denied'));
				 redirect(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : 'pos_screen');
		 } */
		 if (!$this->Settings->default_customer) {
				 $this->session->set_flashdata('warning', lang('please_update_settings'));
				 redirect('settings');
		 ...
		 $register = $this->pos_model->registerData($UserId);
		 $this->session->set_userdata('SessionCreatedDate',$register->date);
		 $CounterDetails = $this->pos_model->GetCounterDetails($register->couter_id);
		 $this->session->set_userdata('CounterName',$CounterDetails->name);
		 if (!$this->session->userdata('register_id')) {

				 if ($register = $this->pos_model->registerData($this->session->userdata('user_id'))) {
						 $register_data = array('register_id' => $register->id, 'cash_in_hand' => $register->cash_in_hand, 'register_open_time' => $register->date);
					//   $this->data['register_data'] =$register_data;
						 $this->session->set_userdata('register_data',$register_data);
			 } else {
						// $this->session->set_flashdata('error', lang('register_not_open'));
						 redirect('pos/open_register');
				 }
		 ...
		 $suspend = $this->input->post('suspend') ? TRUE : FALSE;
		 $OrderKitchen = $this->input->post('Order_kitc_Field') ? TRUE : FALSE;
		 $this->form_validation->set_rules('customer', lang("customer"), 'trim|required');
	 ...
	 $cgst_total_tax = 0;
				 ...
				 $i = isset($_POST['product_id']) ? sizeof($_POST['product_id']) : 0;
				 for ($r = 0; $r < $i; $r++) {
				 $Product_Status = $_POST['product_status'][$r];
						 $item_id = $_POST['product_id'][$r];
						 $real_unit_price = $this->tec->formatDecimal($_POST['real_unit_price'][$r]);
						 $item_quantity = $_POST['quantity'][$r];
						 $item_comment = $_POST['item_comment'][$r];
						 ...
						 if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
								 ...
								 $pr_item_tax = 0; $item_tax = 0; $tax = ""; $cgst_tax = 0;$sgst_tax = 0;$cgst_value = 0;$sgst_value = 0;

			 $gst_details = $this->site->getGstByID($product_details->gst_id);
										 if (isset($product_details->tax) ) {

												 if ($product_details && $product_details->tax_method == 2) {
...
if($Product_Status!='Cancel'){
	...
	$products[] = array(
					 ...
					 'sgst_tax_val' => $pr_sgst_item_tax,
										 ...
										 'real_unit_price' => -abs($real_unit_price),
										 'cost' =>-abs( $product_cost),
										 'product_code' => $product_code,
										 'product_name' => $product_name,
				 'status' => $Product_Status,
										 );

								 $total += ($item_net_price * abs($item_quantity));
}
				 ...
				 if ($this->input->post('order_discount')) {
						 ...
						 if ($opos !== false) {
								 $ods = explode("%", $order_discount_id);
								 $order_discount = $this->tec->formatDecimal((($total ) * (Float)($ods[0])) / 100);
						 } else {
								 $order_discount = $this->tec->formatDecimal($order_discount_id);
						 }
				 } else {
				 ...
				 if($this->input->post('order_tax')) {
						 ...
						 if ($opos !== false) {
								 $ots = explode("%", $order_tax_id);
								 $order_tax = $this->tec->formatDecimal((($total  - $order_discount) * (Float)($ots[0])) / 100);
						 } else {
								 $order_tax = $this->tec->formatDecimal($order_tax_id);
						 }
	 ...
	 if($Last_Pay=='Last_Pay' && $this->tec->roundNumber(($grand_total+$order_tax), $this->Settings->rounding)==$paid)
	 {
		 $status = 'paid';
	 }
				 else{
				 ...
				 $round_total = $this->tec->roundNumber(($grand_total+$order_tax), $this->Settings->rounding);
				 $rounding = $this->tec->formatDecimal($round_total - ($grand_total+$order_tax));
	 ...
	 if ( $this->input->post('OrderStatus')=="1") {
		 ...
		 if($this->input->post('hd_is_del')=='1'){
	 $is_delivery=1;
		 }
		 else{
			 $is_delivery=0;
		 }
		 if ($_POST['paid_by'] == 'Complimentary') {
			 $status = 'Complimentary';
			 // var_dump($_POST);die();
		 }

		$user_id = $this->session->userdata('user_id');
			 $reg =  $this->pos_model->registerData($user_id);
				 $data = array('date' => $date,
						 'customer_id' => $customer_id,
						 'customer_name' => $customer,
						 'total' => $this->tec->formatDecimal($total),
			...
			'table_id' => $this->input->post('table_id'),
			 ...
			 //	  'sale_type' => $this->input->post('hd_sale_type'),
					'delivery_date_time' =>   $this->input->post('deliveryDate'),
						'reg_id' =>  $reg->id,
					 'counter_id' =>    $reg->couter_id,
						 'gstno' =>  $this->input->post('gstno'),
				 ...
				 if (!$eid && !$suspend && !$OrderKitchen && $paid) {
	 ...
	 elseif($eid) {
		 ...
		 if($Last_Pay=='Last_Pay')
		 {
			$amount = $this->tec->formatDecimal($paid > $grand_total ? ($paid - $this->input->post('balance_amount')) : $paid);
				 $date =date('Y-m-d H:i:s');
						 ...
						 $data['paid'] = $amount;
				$data['paid'] =$this->input->post('amount');
		...
		$data['paid'] = $amount;
				$data['paid'] =$this->input->post('amount');
				 ...
				 $products['status']='New';
				 if($Last_Pay!='Update'){
						if($this->pos_model->updateSale_Final($eid, $data, $products,$payment)) {
						 if($Last_Pay!='Last_Pay')
						 {
							 $this->session->set_userdata('rmspos', 1);
							 $this->session->set_flashdata('message', lang("sale_updated"));
		 ...
		 else
		 {
			 $date =date('Y-m-d H:i:s');
				 $data['date'] = $date;
				if($this->pos_model->updateSale_Update($eid, $data, $products,$payment)) {
					 if($Last_Pay=='Update')
					 {
					 $this->session->set_flashdata('message', lang("sale_updated"));
						redirect("pos/pos_screen");
				 ...
				 {
					 $this->session->set_flashdata('error', lang("action_failed"));
					 redirect("pos/pos_screen/?edit=".$eid);
				 }
		 }
...
$data['sale_type']= $this->input->post('hd_sale_type');
						 if($sale = $this->pos_model->addSale($data, $products, $payment, $did)) {
								 $this->session->set_userdata('rmspos', 1);
								 $msg = lang("sale_added");
								 if (!empty($sale['message'])) {
								 ...
								 $this->session->set_flashdata('message', $msg);
			 if ( $this->input->post('OrderStatus')=="1") {
								$redirect_to =  "pos/pos_screen";

						 }
		 else{
			 $redirect_to = $this->Settings->after_sale_page ? "pos/pos_screen" : "pos/view/" . $sale['sale_id'];
		 }

						 //    if ($this->Settings->auto_print) {
						 //        if ( ! $this->Settings->remote_printing) {
						 //            $this->print_receipt($sale['sale_id'], false);
						 //        } elseif ($this->Settings->remote_printing == 2) {
						 //            $redirect_to .= '?print='.$sale['sale_id'];
						 //        }
						 //    }
		 ...
		 else
		 {
			...
			krsort($inv_items);
			 ...
			 $gst = $this->site->getGstByID($row->gst_id);
				 ...
				 $this->data["tcp"] = $this->pos_model->products_count($this->Settings->default_category);
				 $this->data['products'] = $this->ajaxproducts($this->Settings->default_category, 1,"");
				 $this->data['categories'] = $this->site->getAllCategories_POS();
				 $this->data['message'] = $this->session->flashdata('message');
				 $this->data['suspended_sales'] = $this->site->getUserSuspenedSales();
				 ...
				 $bc = array(array('link' => '#', 'page' => lang('pos')));
				 $meta = array('page_title' => lang('pos'), 'bc' => $bc);
				 $this->load->view($this->theme.'pos/pos_screen', $this->data, $meta);
		 }
}

// cake world integration end
