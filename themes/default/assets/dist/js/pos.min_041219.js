...
function loadItems() {
	...
	if (get('spositems')) {
		...
		$.each(spositems, function () {
			...
			product_discount_item = formatDecimal(item_discount * item_qty);

			net_price = formatDecimal(net_price - item_discount);
			var net_price2 = 0;
			net_price2 = net_price;
			...
			if (item_tax_method == 0) {
				...
				net_price2 = (net_price - pr_tax_val);
			...
			tr_html += '<td class="text-right price"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + realitemprice + '"><input class="rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + ds + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + (parseFloat(net_price2)) + '</span></td>';
			...
			// uom
			tr_html += '<td style="width: 10%;" class="text-right discount"><span class="text-right discount" id="discount_' + row_no + '">' + (product_discount_item) + '</span></td>';
			...
			tr_html += '<td class="text-right cgst-tax" style="display:none"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100) + '</span></td>';
			...
			tr_html += '<td class="text-right sgst-tax" style="display:none"><span class="text-right ssubtotal" id="sgst_value_' + row_no + '">' + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.sgst)) / 100) + '</span></td>';

var vat=parseFloat(item.gst.cgst)+parseFloat(item.gst.sgst);
var taxamnt=((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100;
taxamnt=(parseFloat(taxamnt)*2).toFixed(3);

tr_html += '<td style="width: 10%;" class="text-right vat"><span class="text-right ssubtotal" id="cgst_tax_' + row_no + '">' + vat + '</span></td>';
tr_html += '<td style="width: 15%;" class="text-right vat-tax"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + taxamnt + '</span></td>';


			if (item_tax_method == 0) {

				tr_html += '<td class="text-right subtotal"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + ((parseFloat(net_price))).toFixed(3) + '</span></td>';
			} else {
		...
		var g_total = total - parseFloat(order_discount) + parseFloat(gst);
		$("#total-payable").attr('amt',g_total);
		if($('#gstno').val()){
			kerala_flood_cess = 0;
		}
