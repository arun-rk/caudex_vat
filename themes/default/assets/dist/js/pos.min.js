// import { helpers } from './themes/default/assets/dist/js/helpers';
function roundto_5(val=0) {
	factor = 0.005*1000;
	val = val*1000;
...
function loadItems() {
	...
	if (get('spositems')) {
		...
		$.each(spositems, function () {
			...
			var pkg_qty = item.row.pkg_qty;
			var sell_as_pkg = item.sell_as_pkg;
			var pkg ;
			...
			if(sell_as_pkg){
				...
				sell_as_pkg = 'checked';
				pkg = (item_qty/pkg_qty)+' pkg ('+Number(pkg_qty)+'/pkg)';
			}
			else{
				sell_as_pkg = '';
				pkg = ' - ';
			...
			tr_html += '<td  class="text-right sell_as_pkg_td"><span class="text-right pkg_qty" data-id="' + item_id + '"  id="sell_as_pkg_' + row_no + '" >' + ((pkg)) + '</span><input class="sell_as_pkg" '+sell_as_pkg+'  data-id="' + item_id + '" name="sell_as_pkg[]" type="checkbox" value="1"></td>';
...
$(document).ready(function () {
	...
	$(document).on("change", '.rquantity', function () {
		...
		var new_qty = parseFloat($(this).val()),
			...
			// spositems[item_id].row.pkg_qty
			// spositems[item_id].pkg_qty
			if ((spositems[item_id].sell_as_pkg) && ((new_qty % Number(spositems[item_id].row.pkg_qty) ) != 0)) { 
				spositems[item_id].row.qty = Math.ceil(new_qty/Number(spositems[item_id].row.pkg_qty))*Number(spositems[item_id].row.pkg_qty);
				localStorage.setItem('spositems', JSON.stringify(spositems));
				bootbox.alert('Package Sale Enabled ! Quantity must be of package multiple.');
				loadItems();
				// return false;
			}
			else{
	...
	$(document).on("change", '.sell_as_pkg', function () {
// alert();
		var row = $(this).closest('tr');
		// if (!is_numeric($(this).val()) || $(this).val() == 0) {
		// 	loadItems();
		...
		var new_qty = $(this).is(':checked');
		var item_id = $(this).attr('data-id');
		console.log(new_qty);
		console.log(item_id);
		spositems[item_id].sell_as_pkg = new_qty;
		localStorage.setItem('spositems', JSON.stringify(spositems));
		row.find('.rquantity').trigger('change');
		loadItems();
		console.log(row);
	});
	...
	$("#add_item").bind("keydown", function(e){
		var pastedData = $("#add_item").val();
...
data: {},
success: function (data) {
	if(data[0].label != 'No match found'){
		var row = add_invoice_item(data[0]);
	}
...
$("#add_item").val('');
	...
	$('#payment').click(function () {
		if (count <= 1) {
			...
			if (Settings.rounding != 0) {
				...
				$('#twt').text(formatMoney(roundto_5(round_total)) + ' (' + formatMoney(roundto_5(rounding)) + ')');
				$('#quick-payable').text(roundto_5(round_total));
			} else {
				if(gtotal > roundto_5(gtotal))
				{
					sub = gtotal*1000 - roundto_5(gtotal)*1000;
					sub = sub/1000;
					$('#twt').html('BHD.' + roundto_5(gtotal)+'<br><span style=" font-size: 10px; font-weight: 400; " >Rounding (-'+sub+')</span>');
				}
				else if(gtotal < roundto_5(gtotal)){
					sub = roundto_5(gtotal)*1000 - gtotal*1000;
					sub = sub/1000;
					$('#twt').html('BHD.' + roundto_5(gtotal)+'<br><span style=" font-size: 10px; font-weight: 400; " >Rounding (+'+sub+')</span>');
				}
				else{
					$('#twt').text('BHD.' + roundto_5(gtotal));
				}
				$('#quick-payable').text(roundto_5(gtotal));
			}
	...
	$('#amount').on('change',function (e) {
		// alert('qwe');
		round_total = roundto_5(gtotal);
		gtotal = roundto_5(gtotal);
	...
	$(document).on('click', '.quick-cash', function () {
		...
		// console.log(amt);
		// console.groupEnd();
		amt = formatDecimal(amt.split(th).join("")) * 1000 + $pi.val() * 1000;
		amt = amt/1000;
	...
	$('#spos_customer').change(function (e) {
		...
		$.ajax({
			...
			success: function (res) {
				console.log(res.outstanding_amt);
				if(res.outstanding_amt > 0){ 
					res.outstanding_amt = Number(res.outstanding_amt).toFixed(3)
					$('.outstanding').fadeIn() 
					$('.outstanding').text('Outstanding amount : '+res.outstanding_amt) 
					numberMask.updateOptions({max:res.outstanding_amt})
					$('#outstanding_amount').val(res.outstanding_amt) 
					$('#outstanding_amount').data('max-out',res.outstanding_amt) 
					$('#ar_cus_id').val(res.id) 
					// $('#outstanding_amount').attr('max',res.outstanding_amt) 
				}
				else { 
					$('.outstanding').fadeOut() 
					$('.outstanding').text('Outstanding amount : '+0) 
				} 
				$('#gstno').val(res.vat_no);
	...
	$('#submit-sale').click(function () {
		...
		if(in_array($('#paid_by').val(),['card_cheque','cheque_cash'])){
			if($('#cheque_no').val()){
				$('#submit').click();
...
$('#payModal').on('keyup change','.change_ar_amt',function(){
// $('#payModal').on('keyup change','#amount,.cash_amount,.card_amount,.cheque_amount,#paid_by',function(){
 ...
 var c_amount =0;
	if($("#paid_by").val() =="cash_card"){
		...
		c_amount = Number($("#cash_card input.cash_amount").val())*10000 + Number($("#cash_card input.card_amount").val())*10000;
		...
		$('#cash_amount_val').val(0);
		c_amount = Number($("#card_cheque input.card_amount").val())*10000 + Number($("#card_cheque input.cheque_amount").val())*10000;
		...
		c_amount = Number($("#cheque_cash input.cash_amount").val())*10000 + Number($("#cheque_cash input.cheque_amount").val())*10000;
	}
	else{
		c_amount = amount*10000;
	}
	c_amount = c_amount/10000;
	$("#amount").val(c_amount);
	$("#amount").trigger('change');
	$('#balance').text(formatMoney(c_amount - gtotal));
	$('#balance_val').val(formatDecimal(c_amount - gtotal));
	$('#amount_val').val(c_amount);
});


$('body').on('keyup change','.change_ar_amt_out',function(){
// $('#payModal').on('keyup change','#amount,.cash_amount,.card_amount,.cheque_amount,#paid_by',function(){
 var amount = $("#outstanding_amt_model #outstanding_amount").val();
 var c_amount =0;
	if($("#outstanding_amt_model #paid_by").val() =="cash_card"){
		// console.log(Number($("#cash_card input.cash_amount").val()));
		// console.log(Number($("#cash_card input.card_amount").val()));
		$('#outstanding_amt_model #cheque_amount_val').val(0);
		$('#outstanding_amt_model #card_amount_val').val(Number($("#outstanding_amt_model #cash_card input.card_amount").val()));
		$('#outstanding_amt_model #cash_amount_val').val(Number($("#outstanding_amt_model #cash_card input.cash_amount").val()));
		c_amount = Number($("#outstanding_amt_model #cash_card input.cash_amount").val())*10000 + Number($("#outstanding_amt_model #cash_card input.card_amount").val())*10000;
	} else 	if($("#outstanding_amt_model #paid_by").val() =="card_cheque"){
		// console.log(Number($("#card_cheque input.card_amount").val()));
		// console.log(Number($("#card_cheque input.cheque_amount").val()));
		$('#outstanding_amt_model #cheque_amount_val').val(Number($("#card_cheque input.cheque_amount").val()));
		$('#outstanding_amt_model #card_amount_val').val(Number($("#card_cheque input.card_amount").val()));
		$('#outstanding_amt_model #cash_amount_val').val(0);
		c_amount = Number($("#outstanding_amt_model #card_cheque input.card_amount").val())*10000 + Number($("#outstanding_amt_model #card_cheque input.cheque_amount").val())*10000;

	} else 	if($("#outstanding_amt_model #paid_by").val() =="cheque_cash"){
		// console.log(Number($("#cheque_cash input.cash_amount").val()));
		// console.log(Number($("#cheque_cash input.cheque_amount").val()));
		$('#outstanding_amt_model #cheque_amount_val').val(Number($("#cheque_cash input.cheque_amount").val()));
		$('#outstanding_amt_model #card_amount_val').val(0);
		$('#outstanding_amt_model #cash_amount_val').val(Number($("#cheque_cash input.cash_amount").val()));
		c_amount = Number($("#outstanding_amt_model #cheque_cash input.cash_amount").val())*10000 + Number($("#outstanding_amt_model #cheque_cash input.cheque_amount").val())*10000;
	}
	else{
		c_amount = amount*10000;
	}
	c_amount = c_amount/10000;
	var max_out = $("#outstanding_amt_model #outstanding_amount").data('max-out');
	if(max_out >= c_amount){
		$("#outstanding_amt_model #outstanding_amount").val(c_amount);
		$("#outstanding_amt_model #outstanding_amount").trigger('change');
	}
	else{
		alert('Please enter the correct outstanding amount or less');
		$(this).val(0);
	}
	// $('#outstanding_amt_model #balance').text(formatMoney(c_amount - gtotal));
	// $('#outstanding_amt_model #balance_val').val(formatDecimal(c_amount - gtotal));
	// $('#outstanding_amt_model #amount_val').val(c_amount);
});




// $('#payModal').on('change', '.paid_by_out', function () {
