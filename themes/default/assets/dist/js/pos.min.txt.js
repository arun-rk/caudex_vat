...
function toFixedx(num, fixed) {
	fixed = fixed || 0;
	fixed = Math.pow(10, fixed);
	return Math.floor(num * fixed) / fixed;
}

$('#gstno').on('change keyup ', function (e) {
	console.log('Caught');
	console.log(e.target.value);
	//	cal_keralaFloodCess(e.target.value)

});

function cal_keralaFloodCess(gstno) {
	if (gstno) {
		$('.hide_on_gstno').hide();

		kerala_flood_cess = $('#kerala_flood_cess').text();
		kerala_flood_cess = deformat_money(kerala_flood_cess);
		// kerala_flood_cess = parseFloat(kerala_flood_cess.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));

		g_total = $("#total-payable").text();
		g_total = deformat_money(g_total);
		// g_total = parseFloat(g_total.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));
		g_total -= kerala_flood_cess;
		$("#total-payable").attr('amt', g_total);
		$("#total-payable").text(formatMoney(g_total));
		$('#kerala_flood_cess').text(formatMoney(0));
	}
	else {
		$('.hide_on_gstno').show();
		var orignial = parseFloat($("#total-payable").attr('amt'));
		// console.log(orignial);

		total = $('#total').text();
		total = deformat_money(total);
		// total = parseFloat(total.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));
		kerala_flood_cess = total * 0.01;
		$("#kerala_flood_cess").text(formatMoney(kerala_flood_cess));

		// g_total = $("#total-payable").text();
		// g_total = parseFloat(g_total.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));
		orignial += kerala_flood_cess;
		$("#total-payable").text(formatMoney(orignial));
	}
}
function deformat_money(data) {
	var data2 = data.split('.')[2];
	data = data.split('.')[1];
	// data = parseFloat(data.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));    
	if (data) {
		data = parseFloat(data.trim().replace(/[^0-9.-]+/g, ""));
	}
	else {
		data = 0.00;
	}
	return Number(data + '.' + data2);
}
// function deformat_money(data) {
// 	data = data.split('.')[1];
// 	// data = parseFloat(data.split('s.')[1].trim().replace(/[^0-9.-]+/g,""));
...
function loadItems() {
	...
	if (get('spositems')) {
		...
		if (get('spos_discount_all_products') != "0") {
			console.log(get('spos_discount_all_products'));

			dsx = get('spos_discount_all_products');
			spositems = JSON.parse(get('spositems'));
			$.each(spositems, function () {
				this.row.discount = dsx;
			});
			store('spositems', JSON.stringify(spositems));
		}
		spositems = JSON.parse(get('spositems'));
		...
		$.each(spositems, function () {
			...
			product_discount_item = formatDecimal(item_discount * item_qty).toFixed(3);

			net_price = formatDecimal(net_price - item_discount);
			var net_price2 = 0;
			net_price2 = net_price.toFixed(3);
			...
			if (item_tax_method == 0) {
				...
				pr_tax_val = net_price * ((Number(item.gst.cgst) + Number(item.gst.sgst)) / 100);
				// console.log(pr_tax_val);

				var taxs = (Number(item.gst.cgst) + Number(item.gst.sgst));
				var finded = (parseFloat(net_price) / (100 + parseFloat(taxs))) * parseFloat(taxs);
				var finded = finded.toFixed(3);

				//net_price2 = (net_price - pr_tax_val).toFixed(3);
				net_price2 = parseFloat(net_price) - parseFloat(finded);
			...
			var realitemprice = item.row.real_unit_price;
			// if(item.row.tax_method=="0"){
			// 	// (Number(item.gst.cgst)/100)
			// 	// (Number(item.gst.sgst)/100)
			// 	// realitemprice = realitemprice
			// }
			var newTr = $('<tr id="' + row_no + '" class="' + item_id + '" data-item-id="' + item_id + '"></tr>');
			tr_html = '<td class="product"><input name="batchno[]" type="hidden" class="rid" value="' + item.batchno + '"><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="item_comment[]" type="hidden" class="ritem_comment" value="' + item_comment + '"><input name="product_code[]" type="hidden" value="' + item.row.code + '"><input name="product_name[]" type="hidden" value="' + item.row.name + '"><button type="button" class="btn bg-purple btn-block btn-xs edit" id="' + row_no + '" data-item="' + item_id + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')</span></button></td>';
			// <input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatDecimal(item_price) + '">
			tr_html += '<td class="text-right price"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + realitemprice + '"><input class="realupricex" name="real_unit_price_wtgst[]" type="hidden" value="' + net_price2 + '"><input class="rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + ds + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + ((net_price2)) + '</span></td>';
			tr_html += '<td class="Qty"><input name="item_was_ordered[]" type="hidden" class="riwo" value="' + item_was_ordered + '"><input class="form-control input-qty kb-pad text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
			// uom
			tr_html += '<td class="text-right unt"><span class="text-right unt" id="unt_' + row_no + '">' + u + '</span></td>';
			// uom
			tr_html += '<td class="text-right discount"><span class="text-right discount" id="discount_' + row_no + '">' + (product_discount_item) + '</span></td>';
			var vat = parseFloat(item.gst.cgst) + parseFloat(item.gst.sgst);
			if (item_tax_method == 0) {
				taxamnt = (finded) * parseFloat(item_qty);
				var printSgst = formatMoney(parseFloat(taxamnt) / 2);
				var printCgst = formatMoney(parseFloat(taxamnt) / 2);
			}
			else {
				var tot_item_price_ar = (net_price) * (item_qty*100);
				tot_item_price_ar = tot_item_price_ar/100;
				var taxamnt = ((tot_item_price_ar) * (parseFloat(item.gst.cgst)*2));
				var taxamnt = taxamnt / 100;
				// taxamnt = (parseFloat(taxamnt) * 2);
				// taxamnt=(parseFloat(taxamnt)*2).toFixed(3);
				ar_vat += parseFloat(taxamnt);
				console.groupCollapsed('Point xx');
				console.log(taxamnt);
				console.log(ar_vat);
				console.groupEnd();

				var printSgst = formatMoney(((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.sgst)) / 100);
				var printCgst = formatMoney(((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100);
			}

			tr_html += '<td class="text-right cgst" style="display:none" ><span class="text-right ssubtotal" id="cgst_tax_' + row_no + '">' + item.gst.cgst + '</span></td>';
			tr_html += '<td class="text-right cgst-tax" style="display:none"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + printCgst + '</span></td>';
			tr_html += '<td class="text-right sgst" style="display:none"><span class="text-right ssubtotal" id="cgst_tax_' + row_no + '">' + item.gst.sgst + '</span></td>';
			tr_html += '<td class="text-right sgst-tax" style="display:none"><span class="text-right ssubtotal" id="sgst_value_' + row_no + '">' + printSgst + '</span></td>';




			tr_html += '<td class="text-center vat"><span class="text-right ssubtotal" id="cgst_tax_' + row_no + '">' + vat + '</span></td>';
			tr_html += '<td class="text-center vat-tax"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + Math.round(taxamnt * 1000) / 1000 + '</span><input type="hidden"  name="ar_vat[]" value="' + taxamnt + '"></td>';
			// tr_html += '<td class="text-center vat-tax"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + taxamnt.toFixed(3) + '</span><input type="hidden"  name="ar_vat[]" value="' + taxamnt + '"></td>';
			// tr_html += '<td class="text-center vat-tax"><span class="text-right ssubtotal" id="cgst_value_' + row_no + '">' + toFixedx(taxamnt,3) + '</span><input type="hidden"  name="ar_vat[]" value="'+taxamnt+'"></td>';


			if (item_tax_method == 0) {
// console.log(((parseFloat(net_price)) * parseFloat(item_qty)));

				tr_html += '<td class="text-right subtotal"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + Math.round(((parseFloat(net_price)) * parseFloat(item_qty)) * 1000) / 1000 + '</span></td>';

				// tr_html += '<td class="text-right subtotal"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + ((parseFloat(net_price)) * parseFloat(item_qty)).toFixed(3) + '</span></td>';
			} else {

				// console.log(((parseFloat(net_price) * parseFloat(item_qty)) + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100) + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.sgst)) / 100)));

				tr_html += '<td class="text-right subtotal"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + Math.round(((parseFloat(net_price) * parseFloat(item_qty)) + (parseFloat(Math.round(taxamnt * 1000) / 1000))) * 1000) / 1000 + '</span></td>';

				// tr_html += '<td class="text-right subtotal"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + ((parseFloat(net_price) * parseFloat(item_qty)) + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100) + (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.sgst)) / 100)).toFixed(3) + '</span></td>';
			}
			tr_html += '<td class="text-center remove"><i class="fa fa-trash-o tip pointer posdel" id="' + row_no + '" title="Remove"></i></td>';
			newTr.html(tr_html);

			newTr.prependTo("#posTable");
			total += formatDecimal((parseFloat(net_price2)) * parseFloat(item_qty));

			if (item_tax_method == 0) {
				cgst_total = formatDecimal(taxamnt / 2);
				sgst_total = cgst_total;
			}
			else {
				cgst_total += formatDecimal((((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.cgst)) / 100));
				sgst_total += (((parseFloat(net_price) * parseFloat(item_qty)) * parseFloat(item.gst.sgst)) / 100);
			}
			...
			$('#' + row_no).attr('title', 'In Stock :' + item_aqty);
			// console.log($('#' + row_no));
			// console.log(item_aqty);
		...
		// gst = parseFloat(cgst_total) + parseFloat(sgst_total);
		console.log(total+ar_vat);
		var g_total = total - parseFloat(order_discount) + parseFloat(Math.round(gst * 1000) / 1000);
		console.log(g_total);
		g_total = Math.round(g_total * 1000) / 1000;
		$("#total-payable").attr('amt', g_total);
		if ($('#gstno').val()) {
			kerala_flood_cess = 0;
		}
		...
		kerala_flood_cess = 0;  // added by akhil 25-11-2019
		g_total = g_total + kerala_flood_cess;
		// console.log(g_total.toFixed(3));
		grand_total = 'BHD.' + Math.round(g_total * 1000) / 1000;
		// grand_total = 'BHD.' + g_total.toFixed(3);
		// grand_total = formatMoney(g_total);
		$("#ds_con").text('(' + formatMoney(product_discount) + ') ' + formatMoney(order_discount));
		// $("#CGST").text(formatMoney(cgst_total));
		// $("#SGST").text(formatMoney(sgst_total));
		$("#CGST").text('BHD ' + ((ar_vat*1000) / 2))/1000;
		$("#SGST").text('BHD ' + ((ar_vat*1000) / 2))/1000;

		vat = parseFloat(cgst_total) + parseFloat(sgst_total);
		// console.log(ar_vat);
		// console.log(Math.round(ar_vat * 1000) / 1000);

		// $("#VAT").text(formatMoney(ar_vat));
		$("#VAT").text('BHD ' + Math.round(ar_vat * 1000) / 1000);
		kerala_flood_cess = 0;
		$("#kerala_flood_cess").text(formatMoney(kerala_flood_cess));
		//$("#kerala_flood_cess").text(formatMoney(0));
...
$(document).ready(function () {
		...
		if (ds.length != 0) {
			if (apply_to == 'order') {
				...
				if (ds.indexOf("%") !== -1) {
					...
					var g_total_compare=(total + order_tax);
					if(g_total_compare>=order_discount)
					{
				...
				$.each(spositems, function () {
					...
					var unit_price=this.row.unit_price;
					if(unit_price<ds)
					{
						eror++;
					}
				  ...
				  if(eror==0)
				  {
					store('spos_discount_all_products', ds);
					$.each(spositems, function () {
				  ...
				  {
					alert("Failed to apply discount for all items due to high discount value");
				  }
	...
	//href='products/single_barcode/<?= $product_id; ?>'
	...
	$("#add_item").bind("paste", function(e){
		// access the clipboard using the api
		var pastedData = e.originalEvent.clipboardData.getData('text');

$.ajax({
type: 'get',
url: base_url + 'pos/suggestions?term='+pastedData,
dataType: "json",
data: {},
success: function (data) {
var row = add_invoice_item(data[0]);
$("#add_item").val('');
}
});
} );
		...
		$.ajax({
			...
			success: function (data) {
				if (data.result === 'success') {
					spositems[mid] = { "id": mid, "item_id": mid, "label": gcname + ' (' + gccode + ')', "row": { "id": mid, "code": gccode, "name": gcname, "quantity": 1, "price": gcprice, "real_unit_price": gcprice, "tax": 0, "qty": 1, "type": "manual", "discount": "0", "comment": "" }, "gst": { "id": "0", "name": "Gift", "cgst": 0, "sgst": 0 } };
					store('spositems', JSON.stringify(spositems));
					loadItems();
					$('#gcModal').modal('hide');
					$('#gccard_no').val('');
	...
	$('#suspend').click(function () {
		if (count <= 1) {
			...
			$("#susModalLabel").html("Suspend Sale");
	...
	$('#suspend_sale').click(function () {
		...
		if (!ref || ref == '') {
			...
			if (Estimate == false) {
				...
				suspend = $('<span></span>');
				if (sid !== 0) {
					suspend.html('<input type="hidden" name="delete_id" value="' + sid + '" /><input type="hidden" name="estimate" value="yes" /><input type="hidden" name="suspend_note" value="' + ref + '" />');
				} else {
					suspend.html('<input type="hidden" name="estimate" value="yes" /><input type="hidden" name="suspend_note" value="' + ref + '" />');
				}
				suspend.appendTo("#hidesuspend");
			}
	...
	$('#payment').click(function () {
		if (count <= 1) {
			...
			gtotal = formatDecimal(total - order_discount + gst);
			xx = $('#total-payable').text();
			console.log(xx);
			gtotal = deformat_money(xx);
	...
	$('#payModal').on('change', '#paid_by', function () {
		...
		if(p_val=="cash_card")
		{
			...
			$("#cash_card input.cash_amount").val(amt);
			$("#cash_card input.card_amount").val(0);
			$("#cash_amount_val").val(amt);
		...
		else if(p_val=="card_cheque")
		{
			...
			$("#card_cheque input.card_amount").val(amt);
			$("#card_cheque input.cheque_amount").val(0);
			$("#card_amount_val").val(amt);
		...
		else if(p_val=="cheque_cash")
		{
			...
			$("#cheque_cash input.cash_amount").val(amt);
			$("#cheque_cash input.cheque_amount").val(0);
			$("#cash_amount_val").val(amt);
			// $(".cash_amount").val(amt);
		...
		else
		{
			...
			// $(".pcheque").hide();
			$(".cash_amount").val(0);
			$("#cash_amount_val").val(0);
			$(".card_amount").val(0);
			$("#card_amount_val").val(0);
			$(".cheque_amount").val(0);
			$("#cheque_amount_val").val(0);
			$("#amount").change();
		}
	...
	$('#spos_customer').change(function (e) {
		...
		$.ajax({
			type: "get",
			url: base_url + 'customers/get_customersData/' + customerid,
			dataType: "json",
			success: function (res) {
				$('#gstno').val(res.cf2);
				$("#gstno").trigger('change');
				// if(res.cf2){
				// 	$('.hide_on_gstno').hide();
			...
			error: function () {
				// bootbox.alert('Customer data retirval failed');
				// return false;
			}
		});
	});
...
$('#payModal').on('keyup change','#amount,.cash_amount,.card_amount,.cheque_amount,#paid_by',function(){
 var amount = $("#amount").val();
 var c_amount =0;
	if($("#paid_by").val() =="cash_card"){
		// console.log(Number($("#cash_card input.cash_amount").val()));
		// console.log(Number($("#cash_card input.card_amount").val()));
		$('#cheque_amount_val').val(0);
		$('#card_amount_val').val(Number($("#cash_card input.card_amount").val()));
		$('#cash_amount_val').val(Number($("#cash_card input.cash_amount").val()));
		c_amount = Number($("#cash_card input.cash_amount").val())*100 + Number($("#cash_card input.card_amount").val())*100;
	} else 	if($("#paid_by").val() =="card_cheque"){
		// console.log(Number($("#card_cheque input.card_amount").val()));
		// console.log(Number($("#card_cheque input.cheque_amount").val()));
		$('#cheque_amount_val').val(Number($("#card_cheque input.cheque_amount").val()));
		$('#card_amount_val').val(Number($("#card_cheque input.card_amount").val()));
		$('#cash_amount_val').val(0);
		c_amount = Number($("#card_cheque input.card_amount").val())*100 + Number($("#card_cheque input.cheque_amount").val())*100;

	} else 	if($("#paid_by").val() =="cheque_cash"){
		// console.log(Number($("#cheque_cash input.cash_amount").val()));
		// console.log(Number($("#cheque_cash input.cheque_amount").val()));
		$('#cheque_amount_val').val(Number($("#cheque_cash input.cheque_amount").val()));
		$('#card_amount_val').val(0);
		$('#cash_amount_val').val(Number($("#cheque_cash input.cash_amount").val()));
		c_amount = Number($("#cheque_cash input.cash_amount").val())*100 + Number($("#cheque_cash input.cheque_amount").val())*100;
		
	}
	c_amount = c_amount/100;
	$("#amount").val(c_amount);
	$('#balance').text(c_amount);
	$('#amount_val').val(c_amount);
});

function calccashCard()
{
	// var cash_amount=$(".cash_amount").val();
	// if(isNaN(cash_amount) || cash_amount=="") cash_amount=0;
	// var card_amount=$(".card_amount").val();
	// if(isNaN(card_amount) || card_amount=="") card_amount=0;

	// $("#cash_amount_val").val(cash_amount);
	// $("#card_amount_val").val(card_amount);
	// var total=parseFloat(cash_amount)+parseFloat(card_amount);
