...
<div style="grid-area:free"></div> -->
	...
	if ($store) {
			...
			echo " <div class='align_center' style='grid-area:r_header'><span class='semi_text'> DEBIT NOTE </span></div>";
...
<div  class="sales_person tiny_text" style='grid-area:sales_person'><?= lang("Supplier","Supplier").': '. getAfield('name','tec_suppliers','where id = '.$purchases->supplier_id) ?></div>
...
<?php $SGST= 0; ?>
<?php $total= 0; ?>
<?php chrome_log($purchase_items); ?>
<?php foreach( $purchase_items as $row) : ?>
<?php $row = (object)$row; ?>
	<?php $CGST=$CGST+$row->cgst_Tax; ?>
	<?php $SGST=$SGST+ $row->sgst_Tax; ?>									
	...
	<?php if(@$row->tax_method == "0") : ?>
		<td class="tbl_head_item"><?=$this->tec->formatMoney(($row->cost + (@$row->item_tax / $row->quantity))-($row->cgst_Tax+$row->sgst_Tax))?>	</td>
	<?php else : ?>
		<td class="tbl_head_item"><?=$this->tec->formatMoney($row->cost + (@$row->item_tax / $row->quantity))?>	</td>
	<?php endif ?>
	<td class="tbl_head_item"><?=$this->tec->formatQuantity($row->quantity)?>	</td>
	<!-- <td class="tbl_head_item"><?=@$row->discount?> </td> -->
	<?php	$vatval=$row->sgst_Tax+$row->cgst_Tax;?>
