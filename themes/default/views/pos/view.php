...
/* 	'logo logo logo'
	...
	'date date  inv_no '
	'tin tin  vat_no '
	'sales_person sales_person  customer '
	'delivery_place  . . '
...
<div style="grid-area:free"></div> -->
	...
	if ($store) {
			...
			echo " <div class='align_center' style='grid-area:r_header'><span class='semi_text'>".cryptography($store->receipt_header)."</span></div>";
			//echo '<p><b> INVOICE CASH / CREDIT </b></p>';
	}
	?>
<div class="hl" style="grid-area:hl2"></div>
<div class="hl" style="grid-area:hl3"></div>
<div class="inv_no tiny_text" style='grid-area:inv_no'><?= lang('Inv No','Inv No').': '.$inv->id; ?></div>
<div class="date tiny_text" style='grid-area:date'> <?= lang("date","date").': '.$this->tec->hrld($inv->date); ?> </div>
<div  class="sales_person tiny_text" style='grid-area:sales_person'><?= lang("sales_person","sales_person").': '. $created_by->first_name." ".$created_by->last_name; ?></div>
...
<!-- <div class="delivery_place" style='grid-area:delivery_place'><?= lang("Delivery Place","Delivery Place").': '. $inv->hold_ref; ?> </div> -->
	...
	<div class="vat_no" style='grid-area:vat_no'><?= lang("VAT No","VAT No").': '. $inv->gstno; ?></div>

	<table style="grid-area:tb_head" border="1">

<!-- <thead > -->
<tr class="tb_head">
<td class="tbl_head_item">#	</td>
<td class="tbl_head_item">ITEM 	</td>
<td class="tbl_head_item">BHD	</td>
<td class="tbl_head_item">QTY </td>
<td class="tbl_head_item">DIS	</td>
<td class="tbl_head_item">Tax	</td>
<td class="tbl_head_item">AMT</td>
</tr>
<!-- </thead> -->
<tbody class="tb_body" style="grid-area:tb_body" >
<?php $sn= 1; ?>
<?php $CGST= 0; ?>
<?php $SGST= 0; ?>
<?php foreach( $rows as $row) : ?>
	<?php $CGST=$CGST+$row->cgst_tax_val; ?>
	<?php $SGST=$SGST+ $row->sgst_tax_val; ?>									
	<tr class="tb_body_row"  >
	<td class="tbl_head_item"><?=$sn?>	</td>
	<td class="tbl_head_item"> <?=$row->product_name?> 	</td>
	<!-- <td class="tbl_head_item"> <?=$row->product_name.' ('.$row->product_code.')'?> 	</td> -->
	<?php if($row->tax_method == "0") : ?>
		<td class="tbl_head_item"><?=$this->tec->formatMoney(($row->net_unit_price + ($row->item_tax / $row->quantity))-($row->cgst_tax_val+$row->sgst_tax_val))?>	</td>
	<?php else : ?>
		<td class="tbl_head_item"><?=$this->tec->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity))?>	</td>
	<?php endif ?>
	<td class="tbl_head_item"><?=$this->tec->formatQuantity($row->quantity)?>	</td>
	<td class="tbl_head_item"><?=$row->discount?> </td>
	<?php	$vatval=$row->sgst_tax_val+$row->cgst_tax_val;?>
	<td class="tbl_head_item"><?=$vatval?>	</td>
	<td class="tbl_head_item"><?=$this->tec->formatMoney($row->subtotal)?></td>
	</tr>
<?php $sn++?>
<?php endforeach ?>
</tbody>
...
<span>TOTAL :</span>
<span><?=$this->tec->formatMoney($inv->total )?></span>
</div>
<div class="p_dis" style="grid-area:p_dis">
<span>PRODUCT DISCOUNT :</span>
<span><?=$this->tec->formatMoney($inv->product_discount)?></span>
</div>
<div class="o_dis" style="grid-area:o_dis">
<span>ORDER DISCOUNT :</span>
<span><?=$this->tec->formatMoney($inv->order_discount)?></span>
</div>
<div class="vat" style="grid-area:vat">
<span>VAT  :</span>
<span><?=$this->tec->formatMoney($CGST+$SGST)?></span>
</div>
<div class="rounding" style="grid-area:rounding">
<span>ROUNDING  :</span>
<span><?=$this->tec->formatMoney($inv->rounding)?></span>
</div>

<div class="g_total" style="grid-area:g_total">
<span>GRAND TOTAL  :</span>
<?php $grand_total = $inv->grand_total ?>
<?php $grand_total = 	round($grand_total/0.005) * 0.005; ?>
<span style="font-size: 19px">
<span><?=$this->tec->formatMoney($grand_total)?></span>
<!-- <?php if($grand_total > $inv->grand_total) : ?>
	<span style=" font-size: 8px; ">
	Rounding
	<?='(+'.($grand_total - $inv->grand_total).')' ?>
	</span>
<?php elseif($grand_total < $inv->grand_total): ?>
	<span style=" font-size: 8px; ">
	Rounding
...
.paid_details div {
	...
	<div class="paid" style="grid-area:paid">
<?php if($payments) : ?>
	<?php foreach( $payments as $payment) : ?>
			<div class="paid_text" style="grid-area:paid_text">PAID BY :</div>
	<div class="paid_by" style="grid-area:paid_by"><?=ucwords(str_replace('_',' & ' ,$payment->paid_by))?></div>
	<div class="amount_txt" style="grid-area:amount_txt">Paid Amount : </div>
	<div class="amount" style="grid-area:amount"> <?=$this->tec->formatMoney($inv->paid)?></div>
	<!-- <div class="amount" style="grid-area:amount"> <?=$this->tec->formatMoney($inv->paid).' - '.$payment->pos_paid?></div> -->
	<div class="paid_details" style="grid-area:paid_details">
	<?php if($inv->card_amount) : ?>
		<div><span>Card :</span> <span><?=$this->tec->formatMoney($inv->card_amount)?></span> </div>
	<?php endif ?>
	<?php if($inv->cash_amount) : ?>
		<div><span>Cash :</span> : <span><?=$this->tec->formatMoney($inv->cash_amount)?></span> </div>
	<?php endif ?>
	<?php if($inv->cheque_amount) : ?>
		<div><span>Cheque :</span> : <span><?=$this->tec->formatMoney($inv->cheque_amount)?></span> </div>
	<?php endif ?>

	</div>

	<!-- <div class="amount_cash">Amount : 12312</div>
	<div class="amount_card">Amount : 12312</div>
	<div class="amount_chqeue">Amount : 12312</div> -->
	<?php if($payment->note) : ?>
		<div class="note" style="grid-area:note">Note : <?=$payment->note ?></div>
		<?php endif ?>
	<?php if($inv->paid < $inv->grand_total) : ?>
		<div class="due_txt" style="grid-area:due_txt">Due :</div>
		<div class="due" style="grid-area:due"> <?=$this->tec->formatMoney($inv->grand_total - $inv->paid)?></div>
		<?php else : ?>
		<div class="balance_txt" style="grid-area:balance_txt">Balance :</div>
		<div class="balance" style="grid-area:balance"> <?=$this->tec->formatMoney(@$payment->pos_balance)?></div>
		<?php endif ?>
